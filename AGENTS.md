...
- **`BrowserWindow.setPosition`에 소수를 넘기면 renderer가 아니라 main 프로세스가 통째로 죽는다.** (지금은 창을 아예 움직이지 않아 이 경로가 없다. 창을 다시 움직이게 만든다면 되살아난다.) Retina·트랙패드에서 `PointerEvent.screenX`는 소수로 온다. 그냥 반올림하면 커서보다 창이 2배 빨리 가므로, 정수 부분만 보내고 남은 소수는 다음 이벤트로 이월할 것.
...
- **macOS는 투명 창의 크기를 바꿀 때마다 그리는 표면을 새로 잡느라 한 프레임을 비운다.** 패널을 여닫을 때마다 콘텐츠가 잠깐 사라지는 깜빡임으로 보인다. 간헐적이라 재현 조건을 잡기 어렵고, 원인이 우리 코드에 없어서 renderer를 아무리 뒤져도 안 나온다. **창 크기를 상수로 못박아 여닫아 보면 1분 만에 갈린다.** 지금은 창을 작업 영역 전체에 고정해 이 경로를 없앴다. 창 크기를 콘텐츠에 맞추는 방식으로 되돌린다면 같이 되살아난다.
- **한 증상에 원인이 둘일 수 있다.** 이 깜빡임이 그랬다 — 창 리사이즈와, 답을 기다리는 동안 화면을 감추던 구간. 하나를 끄고 증상이 남아 "이건 원인이 아니다"라고 지웠는데 둘 다 원인이었다. **후보를 하나씩 끄지 말고, 조합으로 표를 만들어 볼 것.**
...
- **transform 애니메이션이 도는 요소는 `z-index`를 무시하고 위로 올라온다.** 자기 합성 레이어로 승격되면서 페인트 순서가 뒤집힌다. 뒤에 있어야 할 펫이 패널 앞을 지나갔다. **겹치는 쪽을 `will-change: transform`으로 같이 승격시켜도 낫지 않는다** — 시도했고 증상이 그대로였다. 순서를 다투는 대신 조상에 `clip-path: inset(...)`로 그릴 수 있는 영역 자체를 자를 것. 음수 inset으로 자르고 싶은 변만 고르면 되고, 페인트 순서와 무관하게 먹는다.
