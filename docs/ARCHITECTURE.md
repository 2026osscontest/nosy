...
## 확장 지점
`docs/ADR.md` ADR-010에서 "확장 지점"으로 지정한 곳은 위 "패턴" 섹션의 두 항목과 동일하다. 런타임 플러그인 로더·샌드박스는 v1에서 만들지 않으며, 아래 두 계약이 향후 외부 확장이 들어올 수 있는 지점으로 문서화된 상태다.
- **호스트 주입(`DiagnosticHost`)** — 향후 이 인터페이스를 통해 외부 어댑터가 확장될 수 있다.
- **렌더러 격리(`<PetView>`)** — 향후 이 컴포넌트 계약을 통해 외부 렌더러가 확장될 수 있다.
