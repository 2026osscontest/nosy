...
- FR-010: 메뉴바(상태 표시줄)에 Tray를 상주시키고 다음 4개 메뉴를 제공한다.
  1. **지금 진단하기** — 전체 어댑터를 즉시 실행한다([[drift-detection-spec]] FR-006이 말하는 "수동 실행" 트리거의 진입점)
  2. **펫 숨기기 / 보이기** — 창을 hide/show 한다
  3. **로그인 시 자동 시작** — 체크 토글. `app.setLoginItemSettings`를 갱신한다(FR-008이 요구한 "끄는 방법"의 실제 진입점)
  4. **종료** — 앱을 완전히 종료한다
