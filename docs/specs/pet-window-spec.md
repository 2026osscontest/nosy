...
- FR-010: 메뉴바(상태 표시줄)에 Tray를 상주시키고 다음 5개 메뉴를 제공한다.
  ...
  2. **펫 데려오기** — 창을 화면 가운데로 되돌린다. 드래그는 펫을 화면 안에 가두지만(FR-012), 디스플레이를 뽑거나 해상도가 바뀌면 창이 없어진 화면에 남을 수 있다 — 이 항목이 마지막 복구 경로다
  3. **펫 숨기기 / 보이기** — 창을 hide/show 한다
  4. **로그인 시 자동 시작** — 체크 토글. `app.setLoginItemSettings`를 갱신한다(FR-008이 요구한 "끄는 방법"의 실제 진입점)
  5. **종료** — 앱을 완전히 종료한다
- FR-012: **펫은 언제나 화면 안에 있고, 잘리는 것은 펫 위에 펼쳐진 패널뿐이다.** 드래그 범위는 펫 스프라이트(88×72)가 작업 영역 안에 남는 만큼으로 가둔다. 잘려 나간 방향(위·좌·우)은 판정해 펫의 몸짓으로 알린다.
  - 가두기는 창이 아니라 **펫**을 기준으로 잰다. 창은 가장 넓은 콘텐츠(패널 308px)에 맞춰 380px인데 펫은 그 가운데 88px뿐이라, 창을 기준으로 막으면 펫이 화면 끝에서 146px 앞에 멈춘다
  ...
  - **드래그 중에도 창 높이를 다시 잡는다.** 옮기기만 하면 패널을 편 높은 창이 천장에 걸려 OS가 도로 밀어내고, 펫이 화면 중간에서 멈춘다. 위로 갈수록 창이 낮아지고 패널 위쪽이 잘리는 것이 의도된 동작이다
  - 유도는 **문구나 화살표가 아니라 펫의 몸짓**이다. 위가 잘리면 아래로, 오른쪽이 잘리면 왼쪽으로 4px·8px씩 두 번 튕겼다가 쉬기를 반복한다(2.4초 주기 중 0.72초). 두 방향이 함께 잘리면 대각선으로 합쳐진다
  - 아래쪽 잘림은 없다 — 펫이 화면 아래로 나가지 않기 때문이다
