...
- FR-010: 메뉴바(상태 표시줄)에 Tray를 상주시키고 다음 5개 메뉴를 제공한다.
  ...
  2. **펫 데려오기** — 창을 화면 가운데로 되돌린다. 창이 늘 작업 영역 안으로 밀리므로 펫이 화면 밖으로 사라지지는 않지만(FR-012), 디스플레이를 뽑거나 해상도가 바뀌면 창이 없어진 화면에 남을 수 있다 — 이 항목이 마지막 복구 경로다
  3. **펫 숨기기 / 보이기** — 창을 hide/show 한다
  4. **로그인 시 자동 시작** — 체크 토글. `app.setLoginItemSettings`를 갱신한다(FR-008이 요구한 "끄는 방법"의 실제 진입점)
  5. **종료** — 앱을 완전히 종료한다
- FR-012: **지금 그려진 것 전체가 항상 화면 안에 있는다.** 펫만 떠 있으면 펫이, 패널이 열리면 펫과 패널이 함께. 창은 콘텐츠와 같은 크기이므로 이 규칙은 "창을 작업 영역 안으로 민다"가 된다(`apps/pet/main/panel-layout.ts` `placeBounds`).
  - 펫은 자기 자리(`home`)를 갖는다. **드래그로만 바뀌고**, 패널을 열고 닫는 것으로는 바뀌지 않는다. 패널이 화면 밖으로 나갈 자리면 창이 통째로 밀리며 펫도 home을 떠나지만, 패널을 닫으면 창이 작아져 다시 home에 놓인다
  - 창을 콘텐츠보다 크게 잡으면 안 된다. macOS는 **보이는** 창의 top을 작업 영역 아래로 강제하므로(실측: 요청한 y와 무관하게 `workArea.y`), 펫 위쪽 빈 영역이 그대로 드래그 상한선이 된다. 높이도 폭도 renderer가 잰 콘텐츠에 맞춘다(펫만 104px, 패널 324px)
  - **드래그 중에도 창 높이를 다시 잡는다.** 옮기기만 하면 패널을 편 높은 창이 천장에 걸려 OS가 도로 밀어내고, 펫이 화면 중간에서 멈춘다
  - 밀린 양은 renderer에 전달해 **펫의 몸짓**으로 보여준다. 창과 함께 순간이동하면 왜 움직였는지 읽히지 않는다 — 밀린 방향으로 이동했다가 착지하는 연출로 인과를 보여준다
